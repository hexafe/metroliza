...
Histogram and distribution payloads are built from `src/metroliza/charts/chart_renderer.py`.
IQR/trend payloads are assembled by the summary export path in `src/metroliza/exporting/export_data_thread.py`.
...
`src/metroliza/charts/chart_renderer.py` controls backend selection using `METROLIZA_CHART_RENDERER_BACKEND`
