...
from metroliza.shared.docs_links import (
    DEFAULT_RELEASE_DOCS_REF,
    GITHUB_REPOSITORY_BASE_URL,
    current_release_docs_ref,
    github_blob_url as _github_blob_url,
)
...
def github_blob_url(path: str | Path) -> str:
    """Return a rendered repository URL using the current environment ref."""
    return _github_blob_url(path)
...
def manual_url(manual_key: str) -> str:
    ...
    """
    relative_path = MANUAL_RELATIVE_PATHS[manual_key].lstrip("/")
