...
def _load_dynamic_metric_discovery_rows(
    ...
    *,
    ...
    """
    return read_sql_dataframe(db_file, query, params=params)
...
def _load_dynamic_values_for_records(
    ...
    *,
    ...
    with sqlite_connection_scope(db_file) as conn:
        for field_chunk in chunked_values(field_names, _DYNAMIC_FIELD_BATCH_SIZE):
            record_batch_size = max(1, _SQLITE_BATCH_PARAMETER_TARGET - len(field_chunk))
            for record_chunk in chunked_values(record_ids, record_batch_size):
                ...
                frames.append(read_sql_dataframe("", query, params=params, connection=conn))
    if not frames:
        return pd.DataFrame(columns=["record_id", "field_name", "field_value"])
...
def _append_in_filter(
    ...
    if not values:
        return
    chunks = tuple(chunked_values(tuple(values), _SQLITE_BATCH_PARAMETER_TARGET))
