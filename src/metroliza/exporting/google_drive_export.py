...
from metroliza.shared.log_context import build_google_conversion_log_extra, get_operation_logger
...
def parse_spreadsheet_tab_titles(payload: dict[str, Any]) -> tuple[str, ...]:
    """Extract worksheet titles from a Google Sheets metadata response."""
    sheets = payload.get("sheets")
    if not isinstance(sheets, list):
        raise GoogleDriveResponseError("Google Sheets validation response missing sheets list.")
    ...
    for sheet in sheets:
        ...
        if isinstance(title, str) and title:
            titles.append(title)
    if not titles:
        raise GoogleDriveResponseError("Google Sheets validation response did not include tab titles.")
    return tuple(titles)


def _fetch_converted_tab_titles(access_token: str, spreadsheet_id: str) -> tuple[str, ...]:
    query = urllib.parse.urlencode({"fields": "sheets(properties(title))"})
    request = urllib.request.Request(
        f"{GOOGLE_SHEETS_SPREADSHEET_URL.format(spreadsheet_id=urllib.parse.quote(spreadsheet_id, safe=''))}?{query}",
    ...
    try:
        ...
        raise map_google_network_error("Google Sheets validation failed", exc) from exc
    return parse_spreadsheet_tab_titles(payload)


def _build_sheet_validation_warnings(
    *,
    expected_sheet_names: list[str] | None,
    converted_tab_titles: tuple[str, ...],
) -> tuple[tuple[str, ...], tuple[dict[str, str], ...]]:
    expected_titles = tuple(str(name) for name in expected_sheet_names or [] if str(name).strip())
    if not expected_titles or converted_tab_titles == expected_titles:
        return (), ()

    missing_titles = tuple(title for title in expected_titles if title not in converted_tab_titles)
    unexpected_titles = tuple(title for title in converted_tab_titles if title not in expected_titles)
    detail = {
        "expected": ", ".join(expected_titles),
        "actual": ", ".join(converted_tab_titles),
        "missing": ", ".join(missing_titles),
        "unexpected": ", ".join(unexpected_titles),
    }
    if missing_titles:
        summary = "Google Sheets conversion is missing expected workbook tabs."
    else:
        summary = "Google Sheets conversion tab order differs from the exported workbook."
...
def upload_and_convert_workbook(
    ...
    Args:
        ...
        stage_timeout_seconds: Optional per-stage timeout budgets. Supported keys:
            `auth`, `folder`, `upload`, and `validation`.
