"""Real Qt lifecycle coverage for the tabular industrial loader.

The service seam below delays work in the real ``TabularAnalyticsLoadThread``.
It keeps the production thread, queued signals, delayed progress dialog, and
grouping dialog intact while making terminal ordering deterministic.
"""
...
try:
    ...
    from metroliza.industrial.industrial_workers import TabularAnalyticsLoadThread
...
def test_terminal_progress_cleanup_keeps_callback_replacement_bundle(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert first.dialog._tabular_load_progress == {}
