...
class TestGroupAnalysisWriter(unittest.TestCase):
    def test_group_analysis_sheet_smoke(self):
        ...
        self.assertIn('Printable companion (GitHub PDF)', values)
        self.assertIn('Open Markdown manual', values)
        self.assertIn('Open PDF companion', values)
        ...
        self.assertTrue(any(url[2] == GROUP_ANALYSIS_MANUAL_PDF_GITHUB_URL and url[3] == 'Open PDF companion' for url in worksheet.urls))
