...
| EX-008 | done | maintainer | 2026-03-14 | Stateless export logging helpers around `_build_export_context`, `_log_export_stage`, and `_log_google_issue` were extracted into `modules/export_logging_service.py`, while `ExportDataThread` keeps the existing public method signatures and delegates to the new service.<br><br>- docs touched or explicitly reviewed/no-update-needed rationale:<br>  - docstrings/comments in changed modules: reviewed/updated in the new logging service and thread delegation points.<br>  - `README.md` (if behavior changed): reviewed/no-update-needed (no user-visible behavior change).<br>  - `docs/README.md` index (for new/renamed active docs): reviewed/no-update-needed (internal module extraction only).<br>  - relevant `docs/release_checks/*`: reviewed/no-update-needed (parity procedures unchanged).<br>  - roadmap/execution tracker status: updated in this file.<br>  - naming migration docs if import/module guidance changed: reviewed/no-update-needed (snake_case naming preserved).<br><br>- test review:<br>  - risked behavior: structured logging context shape, google issue suffix formatting, and thread helper call wiring.<br>  - existing coverage: `tests/test_thread_flow_helpers.py` and export/query helper tests.<br>  - tests run/updated: added focused logging-helper and delegation tests in `tests/test_thread_flow_helpers.py`; ran `pytest tests/test_thread_flow_helpers.py tests/test_export_query_service.py`.<br>  - rollback point: revert only `modules/export_logging_service.py` plus delegation import/wrapper edits in `modules/export_data_thread.py` if parity regresses.<br>  - remaining parity gap: release-check parity evidence is now recorded in `docs/roadmaps/exporter_audit_2026_03.md`; further structural work remains Phase-B deferred. | Revisit only in 2026.04+ planning if a Phase-B extraction slice is approved |
...
Phase A parity evidence has been recorded. No additional RC2 extraction slice is required from this tracker; keep EX-004+ deferred until 2026.04+ planning approves a Phase-B structural boundary change.
