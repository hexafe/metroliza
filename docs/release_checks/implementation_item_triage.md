...
| Split `modules/ExportDataThread.py` into orchestration, payload-building, and post-processing modules as a full decomposition. | `defer` | Export pipeline maintainer | 2026.04-rc1 | **Deferred architecture move**: broad module split has integration-risk blast radius beyond rc2 stabilization scope. |
| Refactor `modules/CSVSummaryDialog.py` + `modules/ExportDialog.py` into full UI-state/validation architecture layers. | `defer` | UI/workflow maintainer | 2026.04-rc2 | **Deferred architecture move**: larger boundary reshaping should follow rc2 parity stabilization completion. |
