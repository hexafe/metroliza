...
| 2026.06 RC2 full-repository hardening build | Build `260709` local implementation, integration, and dependency-audit gates passed; package artifact, final push CI, and manual promotion evidence remain pending | Export/OAuth/dashboard safety, parser/report integrity, atomic industrial sync and polling, canonical timestamps, pickle disablement, deterministic shutdown, test isolation, explicit SQLite resource ownership, packaging enforcement, and blocking security policy are implemented. Full local pytest with coverage passed with `2700 passed, 10 skipped, 94 subtests` at `89%`; exact evidence and explicit blockers are recorded in the primary doc. | [`rc2_full_repo_hardening_2026-07-09.md`](./rc2_full_repo_hardening_2026-07-09.md) |
...
- Build `260709` implements the full-repository hardening plan. Local validation
  passed for full offscreen pytest with coverage (`2700 passed, 10 skipped, 6
  warnings, 94 subtests passed`, `89%`), the focused
  release-blocker regression slice (`130 passed`), the exact Qt isolation order
  (`144 passed, 3 subtests`), Ruff, compileall,
  release metadata sync, release hygiene, docs links, whitespace, and the full
  CI-mode security audit with no known dependency vulnerabilities. A real
  packaged artifact, final push CI, and manual promotion evidence remain pending; exact
  evidence is in
